...
export function createResourceRegistry(descriptors) {
  ...
  for (const descriptor of descriptors) {
    if (!descriptor || typeof descriptor.id !== 'string' || !descriptor.id) throw new Error('Registered resources require a stable ID.');
    validateRelativePath(descriptor.path);
    if (registry.has(descriptor.id) || paths.has(descriptor.path)) throw new Error(`Duplicate registered resource: ${descriptor.id}.`);
...
async function snapshot(root, relative) {
  ...
  return { exists: true, bytes: bytes.toString('base64'), sha256: digest(bytes), mode: stat.mode & 0o777 };
}
...
function sameSnapshot(left, right) {
  return left.exists === right.exists && (!left.exists || (left.bytes === right.bytes && left.sha256 === right.sha256));
}
...
async function restoreSnapshot(root, relative, state) {
  ...
  try { await chmod(await safePath(root, relative), state.mode); } catch (error) {
...
function validSnapshot(state) {
  ...
  if (Object.keys(state).length !== 4 || typeof state.bytes !== 'string' || !/^[a-f0-9]{64}$/.test(state.sha256)
    || !Number.isInteger(state.mode) || state.mode < 0 || state.mode > 0o777) return false;
  try { return digest(Buffer.from(state.bytes, 'base64')) === state.sha256; } catch { return false; }
}

function validateJournal(journal, registry) {
  const fields = ['schemaVersion', 'transactionId', 'operation', 'createdAt', 'manifest', 'resources'];
  if (!journal || typeof journal !== 'object' || Array.isArray(journal)
    || Object.keys(journal).length !== fields.length || !fields.every(field => Object.hasOwn(journal, field))
    || journal.schemaVersion !== 1 || !UUID_PATTERN.test(journal.transactionId)
    || typeof journal.operation !== 'string' || !journal.operation || !Number.isFinite(Date.parse(journal.createdAt))
    || !validSnapshot(journal.manifest?.before) || !validSnapshot(journal.manifest?.after)
    || Object.keys(journal.manifest).length !== 3 || journal.manifest.path !== MANIFEST_PATH || !Array.isArray(journal.resources)) {
  ...
  for (const entry of journal.resources) {
    ...
    if (!registered || registered.path !== entry.path || ids.has(entry.resourceId)
      || Object.keys(entry).length !== 4
      || !validSnapshot(entry.before) || !validSnapshot(entry.after)) {
...
export async function readTransactionJournal(root, registry) {
  ...
  let journal;
  try { journal = JSON.parse(raw); } catch { throw new Error('Transaction journal is not valid JSON. Inspect it before manual recovery.'); }
...
async function classify(root, journal) {
  ...
  for (const entry of journal.resources) {
    ...
    const state = sameSnapshot(actual, entry.before) ? 'before' : sameSnapshot(actual, entry.after) ? 'after' : 'conflict';
    resources.push({ entry, actual, state });
  }
  const manifestState = sameSnapshot(manifest, journal.manifest.before) ? 'before'
    : sameSnapshot(manifest, journal.manifest.after) ? 'after' : 'conflict';
  const conflicts = resources.filter(item => item.state === 'conflict').map(item => ({ path: item.entry.path, reason: 'File differs from both recorded transaction states.' }));
  if (manifestState === 'conflict') conflicts.push({ path: MANIFEST_PATH, reason: 'Manifest differs from both recorded transaction states.' });
  return { manifestState, resources, conflicts };
}

export async function inspectTransaction(root, registry) {
  let journal;
  try { journal = await readTransactionJournal(root, registry); }
  catch (error) { return { state: 'invalid', actions: [], conflicts: [{ path: JOURNAL_PATH, reason: error.message }] }; }
  if (journal === null) return { state: 'none', actions: [], conflicts: [] };
  const classified = await classify(root, journal);
  const state = classified.conflicts.length ? 'conflict' : classified.manifestState === 'after' ? 'committed' : 'pending';
  const actions = state === 'pending'
    ? classified.resources.filter(item => item.state === 'after').map(item => ({ action: 'restore', path: item.entry.path }))
    : state === 'committed' ? [{ action: 'finalize', path: JOURNAL_PATH }] : [];
  return { state, transactionId: journal.transactionId, operation: journal.operation, actions, conflicts: classified.conflicts };
}
...
export async function recoverTransaction(root, registry) {
  ...
  return { state: classified.manifestState === 'after' ? 'finalized' : 'rolled-back', transactionId: journal.transactionId, actions, conflicts: [] };
}

export async function applyRegisteredTransaction(root, { operation, registry, changes, manifest, faultBoundary = async () => {} }) {
  if (await readOptional(root, JOURNAL_PATH) !== null) throw new Error('An interrupted transaction requires latchkit recover before mutation.');
  const resources = [];
  const seen = new Set();
  for (const change of changes) {
    ...
    if (seen.has(change.resourceId)) throw new Error(`Duplicate transaction resource: ${change.resourceId}.`);
    ...
    resources.push({ resourceId: descriptor.id, path: descriptor.path, before, after: desiredSnapshot(change.bytes, mode) });
  }
  const manifestBefore = await snapshot(root, MANIFEST_PATH);
  const manifestAfter = desiredSnapshot(Buffer.from(manifest), manifestBefore.exists ? manifestBefore.mode : 0o600);
  const journal = {
    schemaVersion: 1,
  ...
  try {
    for (let index = 0; index < resources.length; index += 1) {
      ...
      const actual = await snapshot(root, entry.path);
      if (!sameSnapshot(actual, entry.before)) throw new Error(`File changed during transaction: ${entry.path}`);
    ...
    try { await recoverTransaction(root, registry); }
    catch (recoveryError) {
      throw new Error(`${error.message} Recovery remains required: ${recoveryError.message}`, { cause: error });
    }
    throw error;
  }
}
