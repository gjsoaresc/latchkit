...
import { generateKeyPairSync, randomBytes, randomUUID, sign, verify, createPublicKey } from 'node:crypto';
...
function validateMetadata(value, raw) {
  ...
  try {
    createPublicKey({ key: Buffer.from(value?.publicKey ?? '', 'base64'), format: 'der', type: 'spki' });
    publicKeyValid = true;
  } catch { /* Invalid keys are ambiguous locks and are never reclaimed automatically. */ }
  const valid = value && typeof value === 'object' && !Array.isArray(value)
    && Object.keys(value).length === fields.length && fields.every(field => Object.hasOwn(value, field))
    && value.schemaVersion === 1 && UUID_PATTERN.test(value.lockId)
    && Number.isInteger(value.pid) && value.pid > 0 && typeof value.startedAt === 'string' && Number.isFinite(Date.parse(value.startedAt))
    && typeof value.hostname === 'string' && value.hostname.length > 0
    && Number.isInteger(value.port) && value.port > 0 && value.port <= 65535
    && typeof value.publicKey === 'string' && publicKeyValid;
  if (!valid) return { state: 'invalid', raw, reason: 'Lock metadata is malformed; inspect it before manual removal.' };
...
async function challenge(metadata) {
  ...
  return new Promise(resolve => {
    ...
    socket.once('connect', () => socket.write(`${JSON.stringify({ lockId: metadata.lockId, nonce })}\n`));
    socket.on('data', chunk => {
      ...
      try {
        ...
        const publicKey = createPublicKey({ key: Buffer.from(metadata.publicKey, 'base64'), format: 'der', type: 'spki' });
        finish(response.lockId === metadata.lockId && verify(null, Buffer.from(`${metadata.lockId}:${nonce}`), publicKey, Buffer.from(response.signature, 'base64')));
      } catch { finish(false); }
    });
...
export async function inspectProjectLock(root) {
  ...
  let parsed;
  try { parsed = JSON.parse(raw); } catch { return { state: 'invalid', raw, reason: 'Lock metadata is not valid JSON; inspect it before manual removal.' }; }
  ...
  const server = net.createServer(socket => {
    ...
    socket.on('data', chunk => {
      ...
      try {
        ...
        if (request.lockId !== lockId || !/^[a-f0-9]{64}$/.test(request.nonce)) return socket.destroy();
        const signature = sign(null, Buffer.from(`${lockId}:${request.nonce}`), privateKey).toString('base64');
...
export async function acquireProjectLock(root) {
  ...
  try {
    ...
    if (error.code === 'EEXIST') {
      ...
      const locked = new Error(existing.state === 'live'
...
export async function withProjectLock(root, operation) {
  ...
  try { return await operation(lock.metadata); }
  finally { await lock.release(); }
}

export async function removeProvenStaleLock(root, inspection) {
  if (inspection.state !== 'stale') throw new Error('Only a proven stale lock can be removed automatically.');
  if (await readOptional(root, LOCK_PATH) !== inspection.raw) throw new Error('Project lock changed during recovery; retry inspection.');
