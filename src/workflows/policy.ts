...
export const POLICY_PROMPT_METADATA = Object.freeze({
  ...
  plan: 'Create a concrete implementation plan against the supplied requirements and exact criterion IDs. Do not edit project files. Put the complete plan in artifact. Inspect the checks value in workflow-context. When checks is non-null, it is the authoritative host-validated acceptance-checks-v1 document: do not rewrite its commands, criteria, or other fields, and leave checks_json empty. When checks is null, put an acceptance-checks-v1 document in checks_json as a JSON-encoded string with this shape: {"schemaVersion":1,"checks":[{"id":"stable-check-id","criterionId":"exact existing criterion ID","label":"observable check","type":"cli","timeoutMs":30000,"outputLimitBytes":262144,"plan":{"executable":"node","args":["--test"],"cwd":"absolute project root"}}]}. Each required criterion must be covered, check IDs must be unique, and proposed commands will be displayed for user approval before they run. Never weaken the criteria.',
  implementation:
