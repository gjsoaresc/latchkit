...
const slug = (scopePath: string): string =>
  scopePath
    .normalize('NFC')
    .replaceAll('/', '-')
...
const scopedPath = (scopePath: string, filename: string): string =>
  scopePath ? `${scopePath}/${filename}` : filename;

export function planProviderExports(
  ...
  const putFile = (relative: string, content: string, scopePath: string) => {
  ...
  if (hasCursor && hasCodex)
    warnings.push({
      ...
      providers: providerIds.filter((id: string) => ['codex', 'cursor', 'cursor-cli'].includes(id)),
      reason:
