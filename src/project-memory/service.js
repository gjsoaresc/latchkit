...
async function normalizeSources(root, sources = [], clock) {
...
export async function listProjectMemory(root, { includeDeleted = false } = {}) {
  const state = await readProjectMemory(root);
  return {
...
export async function inspectProjectMemory(root, id) {
  const state = await readProjectMemory(root);
...
export async function addProjectMemory(root, input, { clock = () => new Date() } = {}) {
...
export async function updateProjectMemory(root, id, input, options = {}) {
...
export async function deleteProjectMemory(
  root,
  id,
  { expectedRevision, clock = () => new Date() } = {},
) {
  return mutate(root, (state) => {
    const memory = state.memories.find((item) => item.id === id && item.deletedAt === null);
    if (!memory)
...
export async function searchProjectMemory(root, query, { limit = 20 } = {}) {
...
export async function exportProjectMemory(root, { clock = () => new Date() } = {}) {
  const state = await readProjectMemory(root);
  return { schemaVersion: 1, exportedAt: iso(clock), memories: active(state).map(result) };
}
export async function importProjectMemory(root, value, { clock = () => new Date() } = {}) {
  validateMemoryExport(value);
  const accepted = [];
  const skipped = [];
...
async function sourceStatus(root, source) {
  const target = path.resolve(root, source.path);
  try {
...
export async function recoverProjectContext(root, { query = '', budget = 4000, provider } = {}) {
