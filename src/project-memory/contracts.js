...
export class ProjectMemoryError extends Error {
  constructor(message, code = 'PROJECT_MEMORY_INVALID', path = '$') {
    super(`${path}: ${message}`);
...
function fields(value, names, required, path) {
  if (!record(value))
...
export function validateProjectMemory(value) {
...
export function parseProjectMemory(raw) {
  try {
    return validateProjectMemory(JSON.parse(raw));
  } catch (error) {
    if (error instanceof ProjectMemoryError) throw error;
    throw new ProjectMemoryError(`Invalid JSON (${error.message}).`, 'PROJECT_MEMORY_INVALID_JSON');
  }
}
