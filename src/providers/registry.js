...
const supported = evidenceUrl => ({ state: 'supported', reason: 'Supported for portable skill export.', versionRange: '*', evidenceUrl });
const baseCapabilities = skillEvidence => ({
  ...
  hooks: {},
  decisions: { blocking: unknown('No provider hook adapter is installed.'), advisory: unknown('No provider hook adapter is installed.') },
...
const definitions = [
  ...
  ['gemini', 'Gemini CLI', 'gemini', '.agents/skills', 'https://geminicli.com/docs/cli/using-agent-skills/'],
...
export const PROVIDERS = Object.freeze(definitions.map(([id, label, command, skillDirectory, evidenceUrl]) => validateProviderContract({
  schemaVersion: PROVIDER_CONTRACT_VERSION, id, label, command, skillDirectory,
  capabilities: baseCapabilities(evidenceUrl),
  verification: { installed: 'unknown', authenticated: 'unknown', configured: 'unverified', endToEnd: 'unverified' },
})));

export function providerById(id) { return PROVIDERS.find(provider => provider.id === id) ?? null; }
