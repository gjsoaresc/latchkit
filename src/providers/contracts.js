...
function validateEvidence(evidence, path) {
  ...
  if (!CAPABILITY_STATES.includes(evidence.state))
...
export function validateProviderContract(contract) {
  ...
  if (contract.schemaVersion !== PROVIDER_CONTRACT_VERSION)
    throw new ProviderContractError(
      `Unsupported provider contract version ${contract.schemaVersion}.`,
      '$.schemaVersion',
      'PROVIDER_CONTRACT_VERSION_UNSUPPORTED',
    );
  for (const field of ['id', 'label', 'command', 'skillDirectory'])
    text(contract[field], `$.${field}`);
  validateCapabilities(contract.capabilities, '$.capabilities');
  if (!record(contract.verification))
...
export function validateCommandPlan(plan) {
  ...
  if (!Array.isArray(plan.args) || plan.args.some((arg) => typeof arg !== 'string'))
    throw new ProviderContractError('Expected string argument array.', '$.plan.args');
  if (plan.cwd !== undefined) text(plan.cwd, '$.plan.cwd');
  if (plan.environment !== undefined && !record(plan.environment))
...
export function validateLifecycleEnvelope(envelope) {
  ...
  if (envelope.schemaVersion !== LIFECYCLE_ENVELOPE_VERSION)
    throw new ProviderContractError(
      'Unsupported lifecycle envelope version.',
      '$.schemaVersion',
      'LIFECYCLE_VERSION_UNSUPPORTED',
    );
  if (!record(envelope.provider))
  ...
  for (const field of ['id', 'version', 'runtime'])
    text(envelope.provider[field], `$.provider.${field}`);
  if (!record(envelope.correlation))
  ...
  for (const field of ['projectId', 'taskId', 'sessionId'])
    text(envelope.correlation[field], `$.correlation.${field}`);
  ...
  if (!LIFECYCLE_EVENT_KINDS.includes(envelope.kind))
  ...
  if (
    !Array.isArray(envelope.decisionModes) ||
    envelope.decisionModes.some((mode) => !DECISION_MODES.includes(mode))
  )
