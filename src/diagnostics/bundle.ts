...
async function buildBundle(root: string) {
  ...
  return {
    ...
    metadata: redact({
      ...
      config: {
        ...
        packCount: 'packs' in config && Array.isArray(config.packs) ? config.packs.length : 0,
      },
...
export async function previewSupportBundle(root: string) {
...
export async function exportSupportBundle(root: string) {
  const bundle = await buildBundle(root);
