...
export class TaskStateError extends Error {
  constructor(message, code = 'TASK_STATE_INVALID', path = '$') {
    super(`${path}: ${message}`);
...
function validateAuthorization(value, path) {
  keys(
    value,
...
function validateProcess(value, path) {
  keys(
    value,
...
function validateTask(value, path) {
  ...
  for (const field of ['authorizations', 'criteria', 'runs', 'checkpoints', 'evidence', 'events']) {
...
export function parseTaskState(raw) {
  ...
  try {
    ...
    throw new TaskStateError(`Invalid JSON (${error.message}).`, 'TASK_STATE_INVALID_JSON');
  }
  return validateTaskState(value);
}

export function validateStableId(value, prefix, path = '$.id') {
