...
export async function safePath(root, relative, finalType = 'file') {
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    if (stat && index < segments.length - 1 && !stat.isDirectory()) throw new Error(`Expected directory: ${relative}`);
    if (stat && index === segments.length - 1 && finalType === 'file' && !stat.isFile()) throw new Error(`Expected regular file: ${relative}`);
    if (stat && index === segments.length - 1 && finalType === 'directory' && !stat.isDirectory()) throw new Error(`Expected directory: ${relative}`);
  }
  return target;
}
...
export async function readOptional(root, relative, encoding = 'utf8') {
  ...
  try { return await readFile(target, encoding); } catch (error) { if (error.code === 'ENOENT') return null; throw error; }
}
