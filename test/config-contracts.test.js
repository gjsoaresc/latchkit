...
test('configuration failures identify a stable code and exact field path', () => {
  const cases = [
    ...
    [{ schemaVersion: 2, providers: ['codex', 'codex'], skills: [], providerSettings: {} }, '$.providers[1]'],
    [{ schemaVersion: 2, providers: ['unknown'], skills: [], providerSettings: {} }, '$.providers[0]'],
    [{ schemaVersion: 2, providers: [], skills: ['unknown'], providerSettings: {} }, '$.skills[0]'],
    [{ schemaVersion: 2, providers: [], skills: [], providerSettings: {}, workflowState: {} }, '$.workflowState'],
    [{ schemaVersion: 2, providers: [], skills: [], providerSettings: { unknown: {} } }, '$.providerSettings.unknown'],
    [{ schemaVersion: 2, providers: [], skills: [], providerSettings: { codex: [] } }, '$.providerSettings.codex'],
  ];
  ...
  assert.throws(() => validateConfig({ schemaVersion: 99 }), error => {
...
test('new projects use v2 and provider settings survive deselection and save', async t => {
  ...
  assert.deepEqual(created, { schemaVersion: 2, providers: ['codex'], skills: ['spec'], providerSettings: {} });
  const configured = {
    ...created,
...
test('v1 migration preview is byte-stable and apply preserves exact original bytes', async t => {
  ...
  assert.equal(await fs.readFile(path.join(root, ...applied.backupPath.split('/')), 'utf8'), original);
...
test('migration orchestration stops after injected backup or config write failures', async () => {
  ...
  let configWrites = 0;
  await assert.rejects(executeMigration('original', migration, {
    readBackup: async () => null,
    writeBackup: async () => { throw new Error('simulated backup failure'); },
    writeConfig: async () => { configWrites += 1; },
  ...
  await assert.rejects(executeMigration('original', migration, {
    ...
    writeBackup: async (_path, raw) => { backup = raw; },
    writeConfig: async () => { throw new Error('simulated replacement failure'); },
...
test('a real Windows replacement denial preserves the active v1 configuration', {
  skip: process.platform !== 'win32' ? 'Uses the Windows read-only file attribute to deny replacement' : false,
}, async t => {
  const root = await temporaryProject(t);
  const original = await installFixture(root, 'config-v1.json');
  const configPath = path.join(root, '.latchkit', 'config.json');
...
test('CLI preview and apply use the same migration contract', async t => {
  ...
  const previewResult = await execFileAsync(process.execPath, [cli, 'migrate', '--dry-run', '--project', root]);
  assert.equal(JSON.parse(previewResult.stdout).status, 'ready');
  assert.equal(await fs.readFile(path.join(root, '.latchkit', 'config.json'), 'utf8'), original);
  const applyResult = await execFileAsync(process.execPath, [cli, 'migrate', '--project', root, '--to', '2']);
