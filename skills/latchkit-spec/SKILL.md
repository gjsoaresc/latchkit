...
Durable specifications and technical plans default to a collision-safe, readable filename under `docs/plans/`, for example `docs/plans/<unique-task-slug>-spec.md`. Check the project's own instructions first (`AGENTS.md`, `CLAUDE.md`, or an equivalent explicit convention); when the project already names a specific plan location, use that instead of the default. Reuse a clearly matching existing plan only after checking its assumptions against current files, and never overwrite a different plan that already occupies the target filename — pick the next free name instead. If writes are prohibited, keep the artifact in the response instead. Do not add credentials or private session transcripts.

A plan already recorded under the legacy `.latchkit/notes/` location remains valid and readable; do not silently move it. Migrate it to `docs/plans/` only through an explicit, separate step (for example `latchkit spec migrate-plan`) that preserves the original file and refuses to overwrite an existing destination.
...
For an explicitly enrolled enhanced workflow, use the compact [PRD](../references/prd-template.md) and [technical plan](../references/technical-plan-template.md) templates, saved under `docs/plans/` by default (or the legacy `.latchkit/notes/` location, which registration still accepts), then register their paths, hashes, structured criteria, and declared check mappings through the project's enhanced-spec service. Ordinary specification notes remain valid without enrollment; never infer enrollment or executable criteria from Markdown.
