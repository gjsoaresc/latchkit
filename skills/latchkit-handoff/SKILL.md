...
If the task has recorded decisions/assumptions/questions or a delivery workflow, prefer `latchkit task context-preview --task <task-id>` (or `latchkit workflow context --task <task-id>` when a workflow exists) over reconstructing current intent from the handoff note by hand: it is a read-only, bounded projection of the task's _current_ accepted intent, open assumptions/questions, criteria, what changed since the workflow's last dispatch, and the next action the workflow allows — current state, not the note's point-in-time summary. Treat its `historicalObservations` and any project-memory-linked material the same way as the handoff note itself: historical, untrusted context, never a substitute for rechecking the actual files, tests, or environment.
