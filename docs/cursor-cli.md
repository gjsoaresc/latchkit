...
Latchkit's `cursor-cli` adapter targets the documented `cursor-agent` executable on macOS, Linux, and Windows through WSL. Upstream does not document a native Windows installer. Latchkit does not install Cursor, sign in, update the CLI, select a model, or modify global preferences.

The adapter creates argument-vector plans only. Interactive sessions use `cursor-agent [prompt]`; non-interactive sessions add `--print`, with `--output-format text|json|stream-json` only when print mode is requested. Resume uses the documented `resume` command for the latest session or `--resume=<chat-id>` for a specific chat. The process runner supplies explicit host-local authorization, bounded output, cancellation, timeout, and Windows shim handling.

`inspect()` returns bounded `--version` and `--help` probes. The default executable is `cursor-agent`. The older `agent` name is considered only when the caller explicitly sets `allowLegacy: true`; Latchkit never guesses a subcommand or silently falls back.
...
- [CLI installation](https://docs.cursor.com/en/cli/installation)
