...
The internal `runWithFccClaudeEnvironment(options, invoke)` helper proves controller ownership and authenticated proxy readiness before calling an existing client runner with child-only `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN`. It clears conflicting inherited Anthropic connection variables, preserves other environment variables, and changes no provider files, CLI arguments, permissions or approvals. The callback must use the existing bounded runner and must never serialize its environment. This helper is deliberately absent from JSON API and CLI output surfaces; no token-printing operation is provided. It does not start an inference call by itself.
