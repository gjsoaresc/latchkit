...
Latchkit's Codex adapter is a plan-and-translation layer. It can inspect a bounded `codex --version` result, create non-executing invocation and resume plans, translate documented hook payloads to the normalized lifecycle envelope, and describe the shared `.agents/skills` and `AGENTS.md` exports.
...
The adapter's evidence is based on the official Codex CLI, AGENTS.md, and hooks documentation checked September 5, 2026:
