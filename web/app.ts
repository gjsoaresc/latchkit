import { createWorkflowConsole } from './workflows.js';
import type {
  ApiError,
  Selection,
  ConsoleState,
...
type PageControls = {
  ...
  'recover-memory': HTMLButtonElement;
  'recovery-provider': HTMLSelectElement;
  'memory-title': HTMLInputElement;
  'memory-kind': HTMLSelectElement;
  'memory-text': HTMLTextAreaElement;
  'memory-query': HTMLInputElement;
  'memory-form': HTMLFormElement;
};
function $<K extends keyof PageControls>(id: K): PageControls[K];
function $(id: string): HTMLElement;
function $(id: string): HTMLElement {
  const element = document.getElementById(id);
  if (!element) throw new Error(`Missing console element: ${id}`);
  return element;
}
...
let state: ConsoleState | undefined;
let savedConfig: Selection | undefined;
let configRevision = '';
let busy = false;
let plan: SyncPreview | undefined;
const API_VERSION = 1;
...
const skillIcons: Record<string, string> = { spec: '◇', fix: '⌁', review: '◎', handoff: '⇢' };
...
function selection(): Selection {
  return {
    ...
    skills: [...document.querySelectorAll<HTMLInputElement>('input[name="skill"]:checked')].map(
      (input) => input.value,
    ),
  };
}
...
function isDirty() {
  return savedConfig !== undefined && comparable(selection()) !== comparable(savedConfig);
}
...
function updateActions() {
  ...
  if (state) $('skill-count').textContent = String(selection().skills.length);
}
...
function renderState({ preserveSelection = null }: { preserveSelection?: Selection | null } = {}) {
  if (!state || !savedConfig) return;
  ...
  $('installed-count').textContent = String(
    state.doctor.providers.filter((provider) => provider.detected).length,
  );
  renderProviders();
  renderSkills();
  if (preserveSelection) {
    ...
    document.querySelectorAll<HTMLInputElement>('input[name="skill"]').forEach((input) => {
...
function renderPlan(nextPlan: SyncPreview) {
  plan = {
...
function renderAcceptance(tasks: ConsoleTask[] = []) {
...
function renderTasks(tasks: ConsoleTask[] = []) {
  ...
  for (const task of tasks) {
    ...
    loadFeedback.addEventListener('click', () =>
      action(async () => {
        const data = await api<Annotations>(`annotations?taskId=${encodeURIComponent(task.id)}`);
    ...
    if (task.checkpoints?.length) {
      ...
      if (last) detail.append(el('p', 'section-note', `Latest checkpoint: ${last.summary}`));
    }
...
function renderMemory(page: MemoryPage) {
...
async function reloadWorkbench() {
  const data = await api<Workbench>('workbench');
...
$('save').addEventListener('click', () =>
  action(async () => {
    ...
    if (state) state.config = savedConfig;
    renderProviders();
    renderSkills();
    showNotice(
      selection().providers.length && selection().skills.length
...
$('preview').addEventListener('click', () =>
  action(async () => {
    renderPlan(await api<SyncPreview>('plan'));
    if (!plan) return;
    showNotice(
...
const workflowConsole = createWorkflowConsole(api, showNotice);

async function reloadState(options: { preserveSelection?: Selection | null } = {}) {
  state = await api<ConsoleState>('state');
  savedConfig = state.config;
  configRevision = state.configRevision;
  renderState(options);
  workflowConsole.configure(
    state.providers.filter((provider) => state?.config.providers.includes(provider.id)),
  );
  await reloadWorkbench();
  await workflowConsole.reload();
}
...
$('search-memory').addEventListener('click', () =>
  action(async () => {
    ...
    renderMemory(await api<MemoryPage>(`memory?query=${encodeURIComponent(query)}`));
  }),
);
...
async function initialize() {
  ...
  try {
    ...
    showNotice(apiError(error).message, true);
