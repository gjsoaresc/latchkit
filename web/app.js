...
} catch { /* The launch URL still works when browser storage is disabled. */ }
...
function selection() {
  return {
    ...savedConfig,
    providers: [...document.querySelectorAll('input[name="provider"]:checked')].map((input) => input.value),
...
function updateActions() {
  ...
  $('apply').disabled = busy || dirty || !plan || plan.conflicts.length > 0 || !plan.changes.some((change) => change.action !== 'unchanged');
  document.querySelectorAll('input[type="checkbox"]').forEach((input) => { input.disabled = busy; });
  $('selection-status').textContent = busy ? 'Working on your workspace…' : !state ? 'Connect this session to load configuration.' : dirty ? 'You have unsaved changes.' : 'Configuration saved. Ready to preview.';
...
function renderProviders() {
  ...
  for (const provider of state.providers) {
    ...
    input.type = 'checkbox'; input.name = 'provider'; input.value = provider.id;
    input.checked = savedConfig.providers.includes(provider.id);
    const top = el('div', 'provider-top');
    top.append(el('span', 'provider-symbol', providerInitials[provider.id] || provider.label.slice(0, 1)), input);
    label.append(top, el('strong', 'provider-name', provider.label));
    const detection = el('span', `detection ${detected ? 'is-detected' : ''}`, detected ? 'Detected' : 'Not detected');
...
function renderSkills() {
  ...
  for (const skill of state.skills) {
    ...
    const input = el('input');
    input.type = 'checkbox'; input.name = 'skill'; input.value = skill.id;
    ...
    label.append(top, el('strong', 'skill-name', skill.label), el('p', 'skill-description', skill.description));
    input.addEventListener('change', invalidatePlan);
    container.append(label);
  }
}
...
function renderState() {
  ...
  $('platform-label').textContent = platformNames[state.doctor.platform] || String(state.doctor.platform);
  $('runtime-label').textContent = `${state.doctor.runtime || 'Node'} · ${state.doctor.node || ''}`;
  $('installed-count').textContent = state.doctor.providers.filter((provider) => provider.detected).length;
  renderProviders();
  renderSkills();
  updateActions();
}
...
function renderPlan(nextPlan) {
  ...
  for (const change of changes) {
    ...
    row.append(el('span', `change-action action-${change.action}`, change.action), el('code', 'change-path', change.path));
    list.append(row);
  }
  if (!changes.length) list.append(el('li', 'empty-plan', 'Everything is already in place. No file changes are needed.'));
  ...
  if (plan.conflicts.length) {
    ...
    for (const conflict of plan.conflicts) $('conflicts').append(el('p', '', `${conflict.path}: ${conflict.reason}`));
  }
  $('plan-note').textContent = plan.conflicts.length ? 'Existing files need your attention. No changes have been applied.' : 'Review these paths before applying the sync.';
  $('plan-panel').classList.remove('hidden');
  $('sync-status').textContent = plan.conflicts.length ? 'Conflict' : changes.length ? 'Pending' : 'In sync';
  $('sync-caption').textContent = plan.conflicts.length ? `${plan.conflicts.length} conflicts to resolve` : changes.length ? `${changes.length} file changes ready` : 'Your workflow is up to date';
  updateActions();
}
...
$('preview').addEventListener('click', () => action(async () => {
  ...
  showNotice(plan.conflicts.length ? 'Sync preview found conflicts. Review the details below.' : 'Sync preview is ready. Review the file changes below.', plan.conflicts.length > 0);
}));
...
async function initialize() {
  if (!token) {
    showNotice('Open the complete session URL printed by Latchkit to connect this dashboard.', true);
