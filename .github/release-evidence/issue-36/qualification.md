...
| -------------------------------- | ----------------------- | ------- | ------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude Code 2.1.258              | pass                    | unknown | unknown | unknown | Official `claude auth status` reported an active first-party session; bounded one-turn smoke exited 0 on native Windows |
| Codex CLI 0.153.2                | pass                    | unknown | unknown | unknown | Bounded one-turn live smoke exited 0; see `provider-codex-windows.json`                                                                             |
| Antigravity CLI                  | unverified              | unknown | unknown | unknown | Official print-mode invocation is contract-tested credential-free; no authenticated run was performed and resume/hooks remain undocumented |
...
- Claude's official `auth status` command reported `loggedIn: true` with the first-party provider. A bounded `Reply only: Latchkit verification.` smoke exited 0 on native Windows/Node 26.8.1; no transcript, credentials, or usage estimate was retained.
- Gemini CLI support was removed after Google's June 18, 2026 retirement of individual-account service; the upstream announcement directs users to Antigravity CLI and distinguishes enterprise/API-key scopes: https://github.com/google-gemini/gemini-cli/discussions/28017. Existing Gemini state is not silently routed to another provider.
- Antigravity CLI's official repository and codelab document `agy -p <prompt>` plus `--output-format json|stream-json`; authentication uses the system keyring or Google Sign-In and workspace trust/permissions remain provider-owned: https://github.com/google-antigravity/antigravity-cli and https://codelabs.developers.google.com/antigravity-cli-hands-on. No install or login was performed. Latchkit therefore records Antigravity as unverified: invocation planning is supported, while hooks and non-interactive resume/session identity are unknown.
...
2. Run the bounded provider verification matrix with authorized accounts on the supported environments. Claude's native Windows smoke now passes; verify Cursor CLI, perform the documented Cursor IDE manual workflow, and record every required OS cell as pass, unsupported, or blocked with evidence. Antigravity requires an authenticated smoke only if maintainers choose it for the release scope; its upstream resume/hooks contract remains incomplete.
